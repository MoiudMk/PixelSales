...
class _ExpensesScreenState extends State<ExpensesScreen>{
 ...
 @override Widget build(BuildContext c)=>Scaffold(appBar:AppBar(title:const Text('المصروفات')),body:data.isEmpty?const Center(child:Text('لا توجد مصروفات')):ListView.builder(itemCount:data.length,itemBuilder:(_,i){final x=data[i];return ListTile(title:Text(x['title']),subtitle:Text(DateFormat('yyyy/MM/dd').format(DateTime.parse(x['created_at']))),trailing:Row(mainAxisSize:MainAxisSize.min,children:[Text('${(x['amount'] as num).toStringAsFixed(2)} د.ل'),IconButton(onPressed:()async{await DatabaseHelper.instance.deleteExpense(x['id']);load();},icon:const Icon(Icons.delete_outline))]);}),floatingActionButton:FloatingActionButton.extended(onPressed:add,icon:const Icon(Icons.add),label:const Text('إضافة مصروف')));
}
